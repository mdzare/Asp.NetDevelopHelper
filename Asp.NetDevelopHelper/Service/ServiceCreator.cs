...
namespace Asp.NetDevelopHelper.Service
{
    public class ServiceCreator
    {
        ...
        public static List<Property> GetProperties(string path, string schema, string table)
        {
            ...
            path = path + $"\\{NameSpaces.Domain}\\Models\\{schema}\\{table}.cs";
        ...
        {
            return IO.IOService.GetDirectories(path + $"\\{NameSpaces.Domain}\\Models");
        }
        public static List<string> GetTables(string path, string schema)
        {
            return IO.IOService.GetFiles(path + $"\\{NameSpaces.Domain}\\Models\\{schema}");
        }
        public static List<string> GetMigrations(string path)
        {
            return IO.IOService.GetFiles(path + $"\\{NameSpaces.Infrastructure}\\Migrations");
        }
        public async void CreateArvinService()
        {
            if (_model.CreateModel)
                IO.IOService.CreateFile(_model.ProjectPath + $"\\{NameSpaces.Domain}\\Models\\{_model.Schema}\\{_model.Table}.cs", _builder.GetModelClass());
            if (_model.CreateDto)
                IO.IOService.CreateFile(_model.ProjectPath + $"\\{NameSpaces.Domain}\\DTOs\\{_model.Schema}\\{_model.Table}Dto.cs", _builder.GetDtoClass());
            if (_model.CreateViewModel)
                IO.IOService.CreateFile(_model.ProjectPath + $"\\{NameSpaces.Domain}\\ViewModels\\{_model.Schema}\\{_model.Table}ViewModel.cs", _builder.GetViewModelClass());

            if (_model.CreateMapping)
                IO.IOService.CreateFile(_model.ProjectPath + $"\\{NameSpaces.Domain}\\Mapping\\{_model.Schema}\\{_model.Table}MappingProfile.cs", _builder.GetMappingClass());

            if (_model.CreateRepository)
            {
                IO.IOService.CreateFile(_model.ProjectPath + $"\\{NameSpaces.Infrastructure}\\Interfaces\\{_model.Schema}\\I{_model.Table}Repository.cs", _builder.GetIRepository());
                
                IO.IOService.CreateFile(_model.ProjectPath + $"\\{NameSpaces.Infrastructure}\\Repositories\\{_model.Schema}\\{_model.Table}Repository.cs", _builder.GetRepository());
            }

            if (_model.CreateService)
            {
                IO.IOService.CreateFile(_model.ProjectPath + $"\\{NameSpaces.Application}\\Interfaces\\{_model.Schema}\\I{_model.Table}Service.cs", _builder.GetIService());
                IO.IOService.CreateFile(_model.ProjectPath + $"\\{NameSpaces.Application}\\Services\\{_model.Schema}\\{_model.Table}Service.cs", _builder.GetService());
            }
            if (_model.CreateController)
                IO.IOService.CreateFile(_model.ProjectPath + $"\\{NameSpaces.Api}\\Controllers\\{_model.Schema}\\{_model.Table}Controller.cs", _builder.GetController());
            if (_model.CreateContextDbset)
            {
                var contextPath = _model.ProjectPath + $"\\{NameSpaces.Infrastructure}\\ArvinContext.cs";
                ...
                if (dbsetIndex == -1)
                {
                    ...
                    IO.IOService.InsertIntoFile(contextPath, 0, $"using {NameSpaces.Domain}.Models.{_model.Schema};\n");
            ...
            if (_model.CreateRelations)
            {
               ...
               var princTables = _model.Relations.Where(x => x.IsSoftRelation).Select(x=> new {x.Schema,  x.Table}).Distinct().ToList();
                ...
                {
                    var repopath = _model.ProjectPath + $"\\{NameSpaces.Infrastructure}\\Repositories\\{_model.Schema}\\{_model.Table}Repository.cs";
                    var principalrepository = _model.ProjectPath + $"\\{NameSpaces.Infrastructure}\\Repositories\\{table.Schema}\\{table.Table}Repository.cs";
            ...
            if (_model.CreateResources)
            {
                ...
                var resourcePath = _model.ProjectPath + $"\\{NameSpaces.Domain}\\Resources\\{_model.Schema}.resx";
                var designerPath = _model.ProjectPath + $"\\{NameSpaces.Domain}\\Resources\\{_model.Schema}.Designer.cs";
            ...
            //var mapPath = _model.ProjectPath + $"\\{NameSpaces.Api}\\Extensions\\ConfigureMapping.cs";
            ...
            //IO.IOService.InsertIntoFile(_model.ProjectPath + $"\\{NameSpaces.Api}\\Extensions\\ConfigureMapping.cs", mapInsertIndex, $"map.AddProfile<{_model.Table}MappingProfile>();\n\t\t\t\t");
            //IO.IOService.InsertIfNotExists(mapPath, 0, $"using {NameSpaces.Domain}.Mapping.{_model.Schema};\r\n");
            ...
            //IO.IOService.InsertIfNotExists(contextPath, 0, $"using {NameSpaces.Domain}.Models.{_model.Schema};\r\n");


            //var servicePath = _model.ProjectPath + $"\\{NameSpaces.Api}\\Extensions\\DependencyInjectionServices.cs";
            ...
            //IO.IOService.InsertIfNotExists(servicePath, 0, $"using {NameSpaces.Application}.Services.{_model.Schema};\r\n");


            //var repoPath = _model.ProjectPath + $"\\{NameSpaces.Api}\\Extensions\\DependencyInjectionRepository.cs";
            ...
            //IO.IOService.InsertIfNotExists(repoPath, 0, $"using {NameSpaces.Infrastructure}.Interfaces.{_model.Schema};\r\n");
        ...
        {
            if (_model.CreateModel)
                IO.IOService.DeleteFile(_model.ProjectPath + $"\\{NameSpaces.Domain}\\Models\\{_model.Schema}\\{_model.Table}.cs");
            if (_model.CreateDto)
                IO.IOService.DeleteFile(_model.ProjectPath + $"\\{NameSpaces.Domain}\\DTOs\\{_model.Schema}\\{_model.Table}Dto.cs");
            if (_model.CreateViewModel)
                IO.IOService.DeleteFile(_model.ProjectPath + $"\\{NameSpaces.Domain}\\ViewModels\\{_model.Schema}\\{_model.Table}ViewModel.cs");

            if (_model.CreateMapping)
                IO.IOService.DeleteFile(_model.ProjectPath + $"\\{NameSpaces.Domain}\\Mapping\\{_model.Schema}\\{_model.Table}MappingProfile.cs");

            if (_model.CreateRepository)
            {
                IO.IOService.DeleteFile(_model.ProjectPath + $"\\{NameSpaces.Infrastructure}\\Interfaces\\{_model.Schema}\\I{_model.Table}Repository.cs");
                IO.IOService.DeleteFile(_model.ProjectPath + $"\\{NameSpaces.Infrastructure}\\Repositories\\{_model.Schema}\\{_model.Table}Repository.cs");
            }

            if (_model.CreateService)
            {
                IO.IOService.DeleteFile(_model.ProjectPath + $"\\{NameSpaces.Application}\\Interfaces\\{_model.Schema}\\I{_model.Table}Service.cs");
                IO.IOService.DeleteFile(_model.ProjectPath + $"\\{NameSpaces.Application}\\Services\\{_model.Schema}\\{_model.Table}Service.cs");

            }
            if (_model.CreateController)
                IO.IOService.DeleteFile(_model.ProjectPath + $"\\{NameSpaces.Api}\\Controllers\\{_model.Schema}\\{_model.Table}Controller.cs");
            if (_model.CreateContextDbset)
            {
                IO.IOService.CutFromFile(_model.ProjectPath + $"\\{NameSpaces.Infrastructure}\\ArvinContext.cs", $"public DbSet<{_model.Table}> {_model.Table} {{ get; set; }}");
            }


            // IO.IOService.CutFromFile(_model.ProjectPath + $"\\{NameSpaces.Api}\\Extensions\\ConfigureMapping.cs", $"map.AddProfile<{_model.Table}MappingProfile>();\n\t\t\t\t");


            //IO.IOService.CutFromFile(_model.ProjectPath + $"\\{NameSpaces.Infrastructure}\\ArvinContext.cs", $"modelBuilder.ApplyConfiguration(new {_model.Table}Configuration());\n\t\t\t");

            // IO.IOService.CutFromFile(_model.ProjectPath + $"\\{NameSpaces.Api}\\Extensions\\DependencyInjectionServices.cs", $"services.AddTransient<I{_model.Table}Service, {_model.Table}Service>();\n\t\t\t");

            // IO.IOService.CutFromFile(_model.ProjectPath + $"\\{NameSpaces.Api}\\Extensions\\DependencyInjectionRepository.cs", $"services.AddTransient<I{_model.Table}Repository, {_model.Table}Repository>();\n\t\t\t");
            if (_model.CreateRelations)
            {
